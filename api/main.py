...
# --- 1. CORS MIDDLEWARE (Required for Chrome Extension) ---
app.add_middleware(
    CORSMiddleware,
...
def compare_tools():
    ...
    if os.path.exists(paths["build_tools"]):
        try: with open(paths["build_tools"], 'r') as f: build_data = json.load(f)
        except: pass
...
def get_python_inodes():
    if os.path.exists(paths["build_inodes"]):
        try: with open(paths["build_inodes"], 'r') as f: return json.load(f)
        except: pass
    return []
...
async def fly_process(payload: FlyRequest):
    ...
    asyncio.create_task(testfly.run_fly_process(
        q, 
        payload.url, payload.cookies, 
        payload.chunk_size, payload.limit_rate, 
        payload.player_clients, payload.wait_time, 
        payload.po_token,
        payload.provider, payload.mode,
        payload.deepgram_key, payload.assemblyai_key
    ))
...
<main>
    ...
    <div id="content">
        ...
        <div id="explorer" class="view active" style="padding:0">
            <table><thead><tr><th style="padding-left:16px">Name</th><th>Size</th><th>Type</th><th>Action</th></tr></thead><tbody id="file-list"></tbody></table>
        </div>
        <!-- TERMINAL -->
        <div id="terminal" class="view">
            <div class="console">
                <div class="output" id="term-out">Welcome to Vercel Shell.</div>
                <div class="input-line"><span style="padding:10px;color:var(--accent)">$</span><input id="term-in" autocomplete="off" autofocus></div>
            </div>
        </div>
        <!-- TESTFLY -->
        <div id="fly" class="view">
            <div class="card">
                <div class="form-grid">
                    ...
                    <div><label>Chunk Size (MB)</label><input id="fly-chunk" value="8M"></div>
                    <div><label>Limit Rate (MB)</label><input id="fly-limit" value="4M"></div>
                    <div><label>Wait Time (s)</label><input id="fly-wait" value="2"></div>
                    <div><label>Clients</label><input id="fly-clients" value="tv,android,ios"></div>
                    <div><label>Provider</label><select id="fly-provider"><option value="assemblyai">AssemblyAI</option><option value="deepgram">Deepgram</option></select></div>
                    <div><label>Mode</label><select id="fly-mode"><option value="debug">Debug (Verbose)</option><option value="data">Data (JSON only)</option></select></div>
                    <div class="full-width"><label>API Key (Leave empty to use Server Env Vars)</label><input id="fly-key" placeholder="Optional if set on server"></div>
        ...
        <div id="stats" class="view">
            <div class="card"><h3 style="margin-top:0">System Runtime</h3><div id="sys-info" style="font-family:var(--mono);font-size:12px;line-height:1.6;color:#888"></div></div>
            <div class="card"><h3 style="margin-top:0">Python Build Inodes</h3><table class="comp-table"><thead><tr><th>Path</th><th>Inode</th><th>Status</th></tr></thead><tbody id="inode-list"></tbody></table></div>
            <div class="card"><h3 style="margin-top:0">Tool Availability</h3><table class="comp-table"><thead><tr><th>Tool</th><th>Build Path</th><th>Runtime Path</th><th>Status</th></tr></thead><tbody id="tool-comp-list"></tbody></table></div>
            <div class="card"><h3 style="margin-top:0">Storage</h3><div id="storage-list"></div></div>
        </div>
    </div>
</main>
<div class="modal" id="file-modal"><div class="modal-content"><div class="modal-head"><span id="modal-title">File Content</span><button class="btn" style="padding:2px 8px" onclick="closeModal()">Close</button></div><div class="modal-body" id="modal-text"></div></div></div>
<script>
    let currentPath = '{project_root}'; let currentSource = 'runtime';
    ...
    function setMode(mode) {{ currentSource = mode; setView('explorer'); document.getElementById('mode-badge').style.display = (mode === 'build') ? 'inline-block' : 'none'; document.getElementById('addr').style.borderColor = (mode === 'build') ? '#f5a623' : '#333'; refresh(); }}
    async function nav(path) {{
        currentPath = path; document.getElementById('addr').value = path;
        const tbody = document.getElementById('file-list'); tbody.innerHTML = '<tr><td colspan="4" style="padding:20px;text-align:center;color:#666">Loading...</td></tr>';
        try {{
            const res = await fetch(`/api/list?path=${{encodeURIComponent(path)}}&source=${{currentSource}}`);
            if(!res.ok) throw await res.text();
            const data = await res.json();
            tbody.innerHTML = '';
            if(data.items.length === 0) tbody.innerHTML = '<tr><td colspan="4" style="padding:20px;text-align:center;color:#444">Empty Directory</td></tr>';
            data.items.forEach(item => {{
                const tr = document.createElement('tr'); tr.className = 'item-row';
                const icon = item.is_dir ? '📁' : '📄';
                const action = (item.is_dir || currentSource === 'build') ? '' : `<a href="/api/download?path=${{encodeURIComponent(item.path)}}" style="color:var(--accent);text-decoration:none;margin-right:10px">⬇</a>`;
                tr.innerHTML = `<td style="padding-left:16px"><span class="file-icon">${{icon}}</span> ${{item.name}}</td><td style="font-family:var(--mono);font-size:11px;color:#888">${{item.size}}</td><td><span class="tag">${{item.ext || 'DIR'}}</span></td><td class="actions">${{action}}${{currentSource === 'runtime' ? `<span onclick="del(event, '${{item.path}}')" style="color:var(--danger);cursor:pointer">✕</span>` : ''}}</td>`;
                tr.onclick = (e) => {{ if(e.target.tagName === 'A' || e.target.tagName === 'SPAN') return; item.is_dir ? nav(item.path) : (currentSource === 'runtime' ? viewFile(item.path) : alert('Cannot view file content from build snapshot')); }};
                tbody.appendChild(tr);
            }});
        }} catch(e) {{ tbody.innerHTML = `<tr><td colspan="4" style="padding:20px;color:red">Error: ${{e}}</td></tr>`; }}
    }}
    function upDir() {{ const parts = currentPath.split('/').filter(p => p); parts.pop(); nav(parts.length ? '/' + parts.join('/') : '/'); }}
    function refresh() {{ nav(currentPath); }}
    async function viewFile(path) {{ const res = await fetch(`/api/view?path=${{encodeURIComponent(path)}}`); const data = await res.json(); document.getElementById('modal-title').innerText = path.split('/').pop(); document.getElementById('modal-text').innerText = data.content || data.error; document.getElementById('file-modal').style.display = 'flex'; }}
    function closeModal() {{ document.getElementById('file-modal').style.display = 'none'; }}
    async function del(e, path) {{ e.stopPropagation(); if(!confirm(`Delete ${{path}}?`)) return; await fetch(`/api/delete?path=${{encodeURIComponent(path)}}`); refresh(); }}
    const termIn = document.getElementById('term-in'); const termOut = document.getElementById('term-out');
    termIn.addEventListener('keypress', async (e) => {{ if(e.key === 'Enter') {{ const cmd = termIn.value; termIn.value = ''; termOut.innerText += `\\n$ ${{cmd}}`; if(cmd === 'clear') {{ termOut.innerText = ''; return; }} const res = await fetch(`/api/shell?cmd=${{encodeURIComponent(cmd)}}`); const data = await res.json(); termOut.innerText += `\\n${{data.out}}`; termOut.scrollTop = termOut.scrollHeight; }} }});
    async function loadStats() {{
        const res = await fetch('/api/stats'); const data = await res.json();
        document.getElementById('sys-info').innerHTML = `OS: ${{data.runtime.os}}<br>Python: ${{data.runtime.python}}<br>AV: ${{data.av}}`;
        const iBody = document.getElementById('inode-list'); iBody.innerHTML = '';
        data.inodes.forEach(i => {{ iBody.innerHTML += `<tr><td><span class="path-cell" title="${{i.path}}">${{i.path}}</span></td><td style="font-family:var(--mono)">${{i.inode}}</td><td>${{i.status}}</td></tr>`; }});
        const tBody = document.getElementById('tool-comp-list'); tBody.innerHTML = '';
        data.tools.forEach(t => {{ const color = t.status.includes('❌') ? '#e00' : '#0a0'; tBody.innerHTML += `<tr><td>${{t.name}}</td><td>${{t.build}}</td><td>${{t.runtime}}</td><td style="color:${{color}}">${{t.status}}</td></tr>`; }});
        const list = document.getElementById('storage-list'); list.innerHTML = '';
        data.storage.forEach(s => {{ list.innerHTML += `<div><b>${{s.label}}</b> ${{s.size}}</div>`; }});
    }}
    async function runFly() {{
        const out = document.getElementById('fly-out'); const provider = document.getElementById('fly-provider').value; const key = document.getElementById('fly-key').value;
        const payload = {{ url: document.getElementById('fly-url').value, cookies: document.getElementById('fly-cookies').value, chunk_size: document.getElementById('fly-chunk').value, limit_rate: document.getElementById('fly-limit').value, wait_time: document.getElementById('fly-wait').value, player_clients: document.getElementById('fly-clients').value, po_token: document.getElementById('fly-token').value, provider: provider, mode: document.getElementById('fly-mode').value, deepgram_key: (provider === 'deepgram') ? key : "", assemblyai_key: (provider === 'assemblyai') ? key : "" }};
        ...
        try {{
            ...
            while(true) {{ const {{value, done}} = await reader.read(); if(done) break; out.innerText += decoder.decode(value, {{stream: true}}); out.scrollTop = out.scrollHeight; }}
            out.innerText += "\\n✅ Stream Closed.";
