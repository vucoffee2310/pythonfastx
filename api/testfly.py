...
async def ship_cargo(session: aiohttp.ClientSession, cargo: Cargo, log_q: asyncio.Queue):
    ...
    try:
        async with session.post(url, headers=headers, data=cargo.buffer) as resp:
            if resp.status >= 400:
                err = await resp.text()
                log(log_q, f"❌ Box #{cargo.index} Failed: {resp.status} {err}")
                return
            ...
            if CONFIG.PROVIDER == "assemblyai":
                ...
                # Try common Deepgram response keys
                res_id = body.get("asset_id") or body.get("url") or body.get("asset")
            
            if not res_id:
                res_id = f"ID_NOT_FOUND (Body: {body})"

            # --- REQUESTED FORMAT: ✅ Box #X: Asset ---
            log(log_q, f"✅ Box #{cargo.index}: {res_id}")

    except Exception as e:
        log(log_q, f"⚠️ Box #{cargo.index} Error: {e}")
    finally:
        cargo.buffer.close()
